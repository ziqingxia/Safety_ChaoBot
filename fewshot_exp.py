...
class InContextLearner():
    ...
    def get_types(self):
        ...
        event_objs = [self.conversation_with_types[name][0]["objective"] for name in event_types]
        event_convs = [self.conversation_with_types[name][0]["conversation"] for name in event_types]
        event_points = [self.conversation_with_types[name][0]["learning_points"] for name in event_types]
        event_ques = [self.conversation_with_types[name][0]["questions"] for name in event_types]
        return event_types, event_descs, event_objs, event_convs, event_points, event_ques
